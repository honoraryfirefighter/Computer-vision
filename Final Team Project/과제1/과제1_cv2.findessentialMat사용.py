...
def insteadTriangulatePoints(P0, P1, pts0, pts1):
    num_points = pts0.shape[1]
...
while True:
    ...
    if ref_image_set and mode in [1, 2]:
        ...
        if len(pts0) >= 5 and len(pts1) >= 5:
            E, mask = cv2.findEssentialMat(pts0, pts1, mtx, cv2.FM_RANSAC)  # OpenCV의 findEssentialMat 사용
            ...
            if mode == 1:
                ...
                P1 = np.hstack((R, t))
                X = insteadTriangulatePoints(mtx @ P0, mtx @ P1, pts0.T, pts1.T)
                X /= X[3]
